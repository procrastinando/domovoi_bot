...
DEFAULT_IMAGE_PROMPT = """You are a specialized visual data analysis AI assistant. Your primary function is to meticulously analyze the provided image and present a structured, factual report. Adhere strictly to the following operating rules:

1.  Structure: Present your analysis in distinct, clearly labeled sections:
    -   `Summary:` A brief, one-sentence overview of the image's content and type (e.g., "A screenshot of a document detailing mythical travelers.").
    -   `Transcribed Text:` Reproduce all text from the image verbatim. Preserve original line breaks and capitalization as accurately as possible.
    -   `Key Elements:` List significant non-textual objects, symbols, icons, or components visible in the image.
    -   `Data Analysis:` If the image contains charts, graphs, or infographics, describe their type and the data they represent.

2.  Transcription Accuracy: If any text is blurry or unreadable, represent it with `[illegible]`. Do not guess.

3.  Formatting:
    -   You must adhere to plain-text formatting. **Never use Markdown** like asterisks for bold or hashtags for headers.
    -   For any data that is clearly tabular in the image, you **must** format your transcription of it as a monospaced, ASCII-style table enclosed in a markdown code block (```). This is the only way to render tables correctly.
"""

DEFAULT_SYSTEM_PROMPT = """You are Domovoi, an AI assistant that focus its answers in giving factual answers and measurable numbers when a numeric answer can be expressed, but flexible and friendly to the user.

Operating rules:

1. Units – always use SI (International System of Units).
2. Formatting – never use Markdown syntax. This means:
 - No leading “#”, “##”, “###”, etc.
 - No asterisks “*” or “**”.
 - No fenced code blocks, bullet lists, or tables.
 - Use only plain‑text lines and indentation (spaces or tabs) to show hierarchy or separation.
3. Structure – each answer should be a single block of plain text.
 - If you need to separate items, use line breaks and simple indentation (for example, two spaces before a sub‑point).
4. Content – give the most precise numeric value you have, state the unit, and, if relevant, a brief confidence range.
5. Fallback – when a precise number is not available, state “no reliable data” and provide the best‑available estimate with its source.

Do not produce any markdown characters, tables, or hashtags. Use only plain text and indentation as described. This prompt must be supplied to the model at every turn to enforce the formatting rule."""
...
async def send_response(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    if use_telegraph_now:
        try:
            ...
            html_content = re.sub(r'<h[1-6].*?>', '<b>', html_content)
            html_content = re.sub(r'</h[1-6]>', '</b><br>', html_content)
...
async def get_groq_completion_with_retry(update: Update, context: ContextTypes.DEFAULT_TYPE, messages: list, model_config: dict, effective_tokens: int):
    ...
    while True:
        ...
        if not api_key:
            await update.message.reply_text("Main API key is not set. Please use /api.")
        ...
        try:
            ...
            if isinstance(e, APIStatusError) and e.status_code == 413:
                await update.message.reply_text("The conversation history is too long. Please start a new chat using /new_chat.")
                raise
            ...
            if fallback_key and attempt_count < max_attempts_with_swap:
                ...
                await send_temp_message(context, user_id, "Primary API key failed. Trying fallback key...", 2)
            ...
            if isinstance(e, RateLimitError):
                ...
                await update.effective_chat.send_message(f"API limit reached. I'll automatically retry in {retry_after} seconds.")
                await asyncio.sleep(retry_after)
                attempt_count = 0
            else:
                logger.error(f"An unexpected API status error occurred for user {user_id}: {e}")
                await update.message.reply_text("An unexpected API error occurred. Please try again.")
                raise
        except Exception as e:
...
async def prepare_messages_for_groq(user_id, history, model_name):
    system_prompt = get_user_data(user_id, 'system_prompt', DEFAULT_SYSTEM_PROMPT)
    telegraph_mode = get_user_data(user_id, 'use_telegraph', 'Never')
                ...
                for item in msg_copy["content"]:
                    if item.get("type") == "image_path":
                        try:
                            ...
                            new_content.append({"type": "text", "text": "[Image not found]"})
                    else:
...
async def set_bot_commands_for_user(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'reasoning_effort' in model_config:
        model_display += f" ({model_config['reasoning_effort']})"
    ...
    tokens_display = f"current: {user_tokens} ({effective_tokens})"
    temp_display = f"current: {model_config.get('temperature', 1.0)}"

    commands = [
        BotCommand("new_chat", "start a new conversation"),
        BotCommand("models", f"{model_display}"),
        BotCommand("web_search", f"{'ON' if get_user_data(user_id, 'web_search_enabled', False) else 'OFF'}"),
        BotCommand("system_prompt", "modify the system prompt"),
        BotCommand("image_prompt", "modify image description prompt"),
        BotCommand("use_telegraph", f"{get_user_data(user_id, 'use_telegraph', 'Never')}"),
        BotCommand("temperature", temp_display),
        BotCommand("max_completion_tokens", tokens_display),
        BotCommand("api", "set main Groq API key"),
        BotCommand("fallback_api", "set fallback Groq API key")
    ]
...
async def new_chat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_data.get("current_chat"):
        archive_key = f"archived_{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"
        user_data.setdefault('archived', {})[archive_key] = user_data["current_chat"]
...
async def api_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await generic_command_handler(update, context, 'awaiting_api_key', f"Current main API key:\n`{escape_markdown_v2(masked)}`\n\nInsert new key\\.")

async def fallback_api_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    key = get_user_data(update.effective_user.id, 'fallback_api_key', 'Not set')
    masked = f"{key[:7]}...{key[-4:]}" if key != 'Not set' else 'Not set'
    await generic_command_handler(update, context, 'awaiting_fallback_api_key', f"Current fallback API key:\n`{escape_markdown_v2(masked)}`\n\nInsert new key\\.")

async def system_prompt_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    prompt = get_user_data(update.effective_user.id, 'system_prompt', DEFAULT_SYSTEM_PROMPT)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("model_"):
        ...
        if model_id == MODELS["GPT-Oss 120B"]:
            ...
            await query.edit_message_text("Select reasoning effort for GPT-Oss 120B.", reply_markup=InlineKeyboardMarkup(keyboard))
            return
        else:
...
async def handle_user_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    state_map = {
        'awaiting_api_key': ('groq_api_key', "Main API key updated!"),
        'awaiting_fallback_api_key': ('fallback_api_key', "Fallback API key updated!"),
        'awaiting_system_prompt': ('system_prompt', "System prompt updated!"),
        'awaiting_image_prompt': ('image_prompt', "Image prompt updated!")
    }

    if state in state_map:
        key, msg = state_map[state]
        value = update.message.text
        if 'api_key' in key and not await asyncio.to_thread(is_api_key_valid_sync, value):
            await cleanup_and_set_state()
            await send_temp_message(context, user_id, "API key not valid.")
            return
        ...
        try:
            ...
            if 0.0 <= temp <= 1.0:
                ...
                set_user_data(user_id, 'selected_model', config)
                await send_temp_message(context, user_id, f"Temperature updated to {temp}")
            else:
                await send_temp_message(context, user_id, "Invalid value.")
        except:
            await send_temp_message(context, user_id, "Invalid format.")
        await cleanup_and_set_state()
        await set_bot_commands_for_user(user_id, context)
        return
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if get_user_data(update.effective_user.id, 'web_search_enabled', False):
        ...
        if results:
            ...
            await send_temp_message(context, update.effective_chat.id, "❗Could not perform web search")
    history = get_user_data(update.effective_user.id, 'current_chat', [])
...
async def handle_audio_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await context.bot.send_chat_action(chat_id=update.effective_chat.id, action='typing')
    oga_path, mp3_path = None, None
    try:
        audio = update.message.voice or update.message.audio
        file = await audio.get_file()
        oga_path = os.path.join(AUDIO_DIR, f"{update.effective_user.id}_{file.file_unique_id}.oga")
        await file.download_to_drive(oga_path)
        mp3_path = os.path.splitext(oga_path)[0] + ".mp3"
        convert_audio_with_ffmpeg(oga_path, mp3_path)
...
async def handle_photo_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        image_path = os.path.join(IMAGE_DIR, f"{user_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}.jpg")
        ...
        if model_config['name'] == MODELS["📷 Llama 4 Maverick"]:
            ...
            if get_user_data(user_id, 'use_telegraph', 'Never') in ['Always', 'Long messages']:
        ...
        await update.message.reply_text("Error with the image.")
    finally:
        if image_path and os.path.exists(image_path):
            os.remove(image_path)

async def handle_unsupported_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Sorry, I can only process text, photo, and audio messages.")
...
def main() -> None:
    ...
    cmd_handlers = {
        ...
        "temperature": temperature_command, "web_search": web_search_command,
